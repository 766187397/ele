<template>
  <div class="pay">
    ...
    <div class="main" v-if="orderInfo">
      ...
      <button @click="addOrder" :disabled="timeOut" class="btn-submit">
        确认支付
      </button>
    </div>
  </div>
  ...
  methods: {
    ...
    pay() {
      const data = {
        ...
        total_fee: 1, // 价格（此处单位为分）
      };
      alert("进入到pay方法中");
      // 支付接口不在之前配置的接口上，所以不能使用axios，而这个接口已经被删除了，支付没有办法操作
      // 微信支付详细访问：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=7_7&index=6
